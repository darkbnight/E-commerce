...
const screenshotDir = path.join(root, 'docs', '测试文档', '商品筛选竞品详情', 'UI');
await mkdir(tempRoot, { recursive: true });
await mkdir(screenshotDir, { recursive: true });

const tempDir = await mkdtemp(path.join(tempRoot, 'competitor-detail-ui-'));
...
const server = await startWorkbenchServer({ port: 4211 });
let browser;

try {
  browser = await chromium.launch({ headless: true });
  const page = await browser.newPage({ viewport: { width: 1440, height: 900 } });
  await page.goto('http://127.0.0.1:4211/results', { waitUntil: 'networkidle' });
  ...
  await page.getByRole('button', { name: '查看竞品详情' }).first().waitFor({ timeout: 10000 });
  await page.screenshot({ path: path.join(screenshotDir, 'selection-default.png'), fullPage: true });

  await page.getByRole('button', { name: '查看竞品详情' }).first().click();
  await page.getByRole('dialog', { name: '竞品详情' }).waitFor({ timeout: 10000 });
  await page.screenshot({ path: path.join(screenshotDir, 'competitor-detail.png'), fullPage: true });
